...
function llenar_pago0() {
    var tipo_pago = $("#id_tipo_pago0").val();
    if (tipo_pago == "1") {
        $("#campos4").show();
        $("#campos5").hide();
    } else {
...
function llenar_factura0() {
    var factura = $("#facturacion0").val();
    if (factura <= "1") {
        $("#campos6").show();
    } else {
        $("#campos6").hide();
...
function llenar_pago3() {
    var tipo_pago = $("#id_tipo_pago3").val();
    if (tipo_pago == "1") {
        $("#campos7").show();
        $("#campos8").hide();
    } else {
...
function llenar_factura3() {
    var factura = $("#facturacion3").val();
    if (factura <= "1") {
        $("#campos9").show();
    } else {
        $("#campos9").hide();
...
function llenar_sub_mod0(){
    var id_modalidad = $('#modalida_rendi').val();
    ...
    $.ajax({
        ...
        success: function(response){
            $('#id_sub_modalidad0').find('option').not(':first').remove();
            $.each(response, function(index, data){
                $('#id_sub_modalidad0').append('<option value="'+data['descripcion']+'">'+data['descripcion']+'</option>');
            });
        }
    });
}
function consultar_rif0(){ //PARA consulta contratista dentro del modal
    var rif_b = $('#rif_b0').val();
    if (rif_b == ''){
        swal({
            title: "¡ATENCION!",
            text: "El campo no puede estar vacio.",
            ...
            success: function(data){
                if (data == null) {
                   ...
                   // $('#exitte').val(0);
                    ...
                    $('#sel_rif_nombre').val(data['rifced']);
                    $('#nombre_conta_0').val(data['nombre']);
...
//////////paridad factura
        ...
        var selc_iva_rendi55 = $('#selc_iva_rendi2').val();
        var separar1 = selc_iva_rendi55.split("/");
        var porcentaje1 = separar1['0'];
            // calcular  monto_factura_rend5
        var monto_factura_rend55 = base_imponible_rendi*porcentaje1;
        ...
        $('#monto_factura_rend').val(monto_factura_rend501);
        // calculo total pago
                    

                
        var montoend5 = $('#monto_factura_rend').val();
        ...
        //Calculo total pago
        var total_pago_rendi51 = Number(base_imponible_rendi) + Number(montoend55);
        var total_pago_rendi52 = Intl.NumberFormat("de-DE").format(total_pago_rendi51);
        $('#total_pago_rendi').val(total_pago_rendi52);
       
        var paridad_rendi_factura55 = $('#paridad_rendi_factura').val();
        ...
        var subtotal_rendi_facturas5 = base_imponible_rendi/paridad_rendi_factura5;
        ...
        $('#subtotal_rendi_factura').val(subtotal_rendi_factura35);
...
function guardar_proc_pago(){
  //  var rendido  = $("#rendidoa").val();
    ...
    swal
        ...
        //         return 0;
        //  }  
    //     if (document.guardar_proc_pag.modalida_rendi.selectedIndex==0){
    //         alert("Debe seleccionar un PROCEDIMIENTO DE CONTRATACIÓN.")
    //         document.guardar_proc_pag.modalida_rendi.focus()
    //         return 0;
    //  }
    //     if (document.guardar_proc_pag.selc_tipo_doc_contrata.selectedIndex==0){
    //         alert("Debe seleccionar un TIPO DOCUMENTO CONTRATACIÓN.")
    //         document.guardar_proc_pag.selc_tipo_doc_contrata.focus()
    //         return 0;
    //  }
    //     if (document.guardar_proc_pag.facturacion5.selectedIndex==0){
    //         alert("Debe seleccionar una opción ¿Desea Registrar Facturación y Pago?.")
    //         document.guardar_proc_pag.facturacion5.focus()
    //         return 0;
    //  }
...
function calculos_rendi_obritas(){
    ...
    var iv = $('#IV').val();
       ...
       //Calculo monto_factura_rend3

       var base_imponible_rendi52 = $('#base_imponible_rendi3').val();
       ...
       var selc_iva_rendi55 = $('#selc_iva_rendi4').val();
       var separar1 = selc_iva_rendi55.split("/");
       var porcentaje1 = separar1['0'];
           // calcular  monto_factura_rend5
       ...
       var monto_factura_rend500 = parseFloat(monto_factura_rend55).toFixed(2);
       var monto_factura_rend501 = Intl.NumberFormat("de-DE").format(monto_factura_rend500);
       $('#monto_factura_rend3').val(monto_factura_rend501);

            // calculo total pago
            

        
            var montoend5 = $('#monto_factura_rend3').val();
            var montoend51 = montoend5.replace('.', "");
            var montoend52= montoend51.replace('.', "");
            var montoend53 = montoend52.replace('.', "");
            var montoend54 = montoend53.replace('.', "");
            var montoend55 = montoend54.replace(',', "."); //hasta aca para poder sumar '??
             ...
             var total_pago_rendi52 = Intl.NumberFormat("de-DE").format(total_pago_rendi51);
             $('#total_pago_rendi3').val(total_pago_rendi52);
    ...
    function llenar_sub_mod3(){
        var id_modalidad = $('#modalida_rendi3').val();
        // var base_url = window.location.origin+'/asnc/index.php/evaluacion_desempenio/llenar_sub_modalidad';
        var base_url = '/index.php/evaluacion_desempenio/llenar_sub_modalidad';
        ...
        $.ajax({
            ...
            success: function(response){
                $('#id_sub_modalidad3').find('option').not(':first').remove();
                $.each(response, function(index, data){
                    $('#id_sub_modalidad3').append('<option value="'+data['descripcion']+'">'+data['descripcion']+'</option>');
                });
            }
        });
    }
    function consultar_rif3(){ //PARA consulta contratista dentro del modal
        var rif_b = $('#rif_b3').val();
        if (rif_b == ''){
            swal({
                title: "¡ATENCION!",
                text: "El campo no puede estar vacio.",
                ...
                success: function(data){
                    if (data == null) {
                       ...
                       // $('#exitte').val(0);
                        ...
                        $('#sel_rif_nombre3').val(data['rifced']);
                        $('#nombre_conta_3').val(data['nombre']);
        ...
        swal
            ...
            .then((result) => {
                if (total_rendido  > totalprogrmado ) {
                    alert("El Monto Total Contratado es mayor al Ingresado en la Programación anual, para continuar debe  Reprogramar.")
                    }
            //     if (document.guardar_proc_pag.dolar.value.length==0){
            //         alert("No Puede dejar el campo Valor Dolar vacio, Ingrese un Monto")
            //         document.guardar_proc_pag.dolar.focus()
