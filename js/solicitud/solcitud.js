...
function save(event) {
    ...
    // Validar el reCAPTCHA directamente
    ...
    if (recaptcha_response.length === 0) {
        swal.fire({
            ...
            type: 'warning', // Usar 'type' si es SweetAlert (no v2)
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        swal.fire({
                            ...
                            html: 'ESTA PLANILLA DEBE REMITIRSE FIRMADA POR LA MAXIMA AUTORIDAD O CUENTADANTE AL SIGUIENTE CORREO clavesi@snc.gob.ve, números corporativos: 0426-5654730/0426-5654740<br>Número de Solicitud: ' + response.id_solicitud, // Usar response.id_solicitud
                            type: 'success',
                            showCancelButton: false,
                            confirmButtonColor: '#3085d6',
                            confirmButtonText: 'LA DESCARGA DEBIO INICIAR, SINO DAR CLIC AQUI'
                        }).then((resultSuccess) => { // Cambié nombre de result a resultSuccess
                            if (resultSuccess.value) {
                                window.location.href = base_url_3 + response.id_solicitud;
                            }
                        });
                        setTimeout(function() {
                            window.location.href = base_url_3 + response.id_solicitud;
                        ...
                        swal.fire({
                            title: 'Error',
                            text: response.message, // Mostrar el mensaje de error del servidor (incluyendo CAPTCHA/Validación)
                            type: 'error',
                            confirmButtonColor: '#3085d6',
                            confirmButtonText: 'Ok'
                        });
