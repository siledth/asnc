...
class Comision_contrata_model extends CI_model
{
    ...
    ///cambio para que se vea dependiendo de la desendenmcia del rif padre
    ...
    public function check_logger_commission_snc($rif_organoente)
    {
        $this->db->select('
        c.id_comision,
        c.rif_organoente,
    ...
    ');
        ...
        // Lógica para filtrar por rif o descendencia
        if ($rif_organoente != 'G200024518') {
            // Obtenemos la descendencia primero
            $sql_descendants = "WITH RECURSIVE descendants AS (
            SELECT 
                organoente.rif
            FROM public.organoente
            WHERE organoente.rif = ?
            UNION ALL
            SELECT 
                o.rif
            FROM public.organoente o
            JOIN descendants d ON o.id_organoenteads = (
                SELECT id_organoente FROM public.organoente WHERE rif = d.rif
            )
        )
        SELECT rif FROM descendants;";

            $query = $this->db->query($sql_descendants, array($rif_organoente));
            $descendencia_rifs = $query->result_array();

            $rifs_a_filtrar = array_column($descendencia_rifs, 'rif');

            if (!empty($rifs_a_filtrar)) {
                $this->db->where_in('c.rif_organoente', $rifs_a_filtrar);
            }
        }
    ...
    public function check_logger_commissionsnctodos($rif_organoente)
    {
        $this->db->select('c.id_comision,c.rif_organoente,c.observacion,c.id_status,c.snc,c.tipo_comi, c.fecha_desig, c.num_acto, c.fecha_acto, 
        c.acto_adm, c.dura_com_desde, c.dura_com_hasta, stu.desc_status, nto.desc_status_snc, act.desc_acto_admin, c.snc');
        $this->db->from('comisiones.comision c');
        ...
        // Lógica para filtrar por rif o descendencia
        if ($rif_organoente != 'G200024518') {
            // Obtenemos la descendencia primero (la misma lógica de la consulta recursiva)
            $sql_descendants = "WITH RECURSIVE descendants AS (
            SELECT 
                organoente.rif
            FROM public.organoente
        ...
        // Si el rif es G200024518, no se agrega ninguna cláusula `where`,
        // por lo que se mostrarán todos los resultados.
    ...
    function consulta_mib_s_j($data3)
    { //da totales agrupados por bienes, servicio, obras
    ...
    function consulta_mib_tec($data4)
    { //da totales agrupados por bienes, servicio, obras
    ...
    function consulta_mib_s_t($data3)
    { //da totales agrupados por bienes, servicio, obras
    ...
    function consulta_mib_fin($data6)
    { //da totales agrupados por bienes, servicio, obras
    ...
    function consulta_mib_s_fin($data7)
    { //da totales agrupados por bienes, servicio, obras
    ...
    function consulta_mib_secre($data9)
    { //da totales agrupados por bienes, servicio, obras
    ...
    function consulta_mib_s_decre($data10)
    { //da totales agrupados por bienes, servicio, obras
    ...
    { //mostrar para editar informacion academica de los miembros
    ...
    function consulta_infomr_modal_exp_miembro($data)
    { //mostrar para editar informacion experiencia 5 años de los miembros
    ...
    public function editar_informacion_academica($data)
    ...
    public function editar_modal_exp_miembro($data)
    {  // edita la informacion academica del miembro de comision
