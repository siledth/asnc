...
class Publicaciones_model extends CI_model
{
	...
	public function consulta_llamados_prorrogados()
	{
		$this->db->select('numero_proceso, denominacion_proceso, fecha_tope, fecha_disponible_llamado, estatus, 
				id_llcestatus, fecha_fin_llamado');
		$this->db->from('public.llamado_concurso_view');
		$this->db->where('id_llcestatus', 1);
		$this->db->where('fecha_fin_llamado >=', date('Y-m-d')); // Fecha mayor o igual a hoy
		$this->db->where('fecha_fin_llamado <=', date('Y-m-d', strtotime('+1 day'))); // Fecha menor o igual a mañana					
		$query = $this->db->get();
		return $result = $query->result_array();
	}
	...
	public function guardar_anulaciones($anular, $numero_proceso, $numero_proceso2)
	{
		...
		$query = $this->db->query(
			"insert into historico_llamado_concurso (  rif_organoente, numero_proceso, id_modalidad, id_mecanismo, 
	...
	public function guardar_Prorroga($data)
	{
		...
		$id1 = $response4['id'] + 1;
		$query = $this->db->query(
			"insert into historico_llamado_concurso (id,rif_organoente, numero_proceso, id_modalidad, id_mecanismo,
	...
	public function consultar_llamados_internos($date, $rif, $limit, $offset, $search = NULL)
	{
		$this->db->select('rif_organoente, organoente, numero_proceso, fecha_disponible_llamado as formatted_date, estatus, objeto_contratacion, denominacion_proceso, estado, fecha_disponible_llamado');
		$this->db->from('llamado_concurso_view');
		$this->db->order_by("fecha_disponible_llamado", "desc");
		//$this->db->where('rif_organoente', $rif);
		if ($rif == 'G200024518') {
			// Aplicar búsqueda solo si $search no es NULL
			if ($search) {
				$this->db->group_start();
				$this->db->like('rif_organoente', $search);
				$this->db->or_like('organoente', $search);
				$this->db->or_like('numero_proceso', $search);
				$this->db->or_like('estatus', $search);
				$this->db->or_like('objeto_contratacion', $search);
				$this->db->or_like('denominacion_proceso', $search);
	...
	public function count_llamados_internos($date, $rif, $search = NULL)
	{
	...
	function consulta_llamado2($rif_organoente, $numero_proceso)
	{
		$sql = "
        SELECT 
            c.numero_proceso, c.id_modalidad, c.id_mecanismo, c.id_objeto_contratacion, 
            c.dias_habiles, c.fecha_llamado, c.fecha_disponible_llamado, c.fecha_fin_aclaratoria, 
            c.fecha_tope, c.fecha_fin_llamado, c.denominacion_proceso, c.descripcion_contratacion, 
            c.hora_desde, c.hora_hasta, c.id_estado, c.id_municipio, c.direccion, 
            c.hora_desde_sobre, c.id_estado_sobre, c.id_municipio_sobre, c.direccion_sobre, 
            c.lugar_entrega, c.observaciones, c.idestatus, c.fecha_inicio_aclaratoria, 
            c.mecanismo, c.modalidad, c.objeto_contratacion, c.estado, c.municipio,
        ...
        LEFT JOIN public.organoente oe ON oe.rif = c.rif_organoente
        LEFT JOIN public.organoente oe_ads ON oe_ads.id_organoente = oe.id_organoenteads
        LEFT JOIN public.modalidad m ON m.id_modalidad = c.id_modalidad
        LEFT JOIN public.mecanismo cn ON cn.id_mecanismo = c.id_mecanismo
        JOIN public.objeto_contratacion obj ON obj.id_objeto_contratacion = c.id_objeto_contratacion       
	...
	// 		public function enviar_snc1($data) {
	//     // Datos a actualizar
	//     $data1 = array(
	//         'estatus' => '5', // Estatus "enviado al SNC"
	//         'fecha_disponible_llamado' => date('Y-m-d'), // Fecha actual desde PHP
	//     );

	//     // Usamos set() para PostgreSQL maneje el cálculo de la fecha
	//     $this->db->set('fecha_fin_llamado', "CURRENT_TIMESTAMP + INTERVAL '1 day'", FALSE);

	//     // Filtramos por número de proceso
	//     $this->db->where('numero_proceso', $data['numero_proceso']);

	//     // Ejecutamos la actualización
	//     $update = $this->db->update('public.llamado_concurso', $data1);
	//     return true;
	// }
	public function enviar_snc1($numero_proceso)
	{
		// Validar entrada
		...
		// Retorna 1 si la actualización fue exitosa, 0 si falló
		return $this->db->update('public.llamado_concurso', $data1) ? 1 : 0;
	}
	public function check_logger_accion_sncn($cedula)
	{

		$query = $this->db->query("
        select c.id_accion, c.numero_proceso, c.id_accion_cargar, c.id_articulo_113,
		c.observacion_desierto, c.adjudicado, c.sel_rif_nombre, c.nombre_contratista,
		c.rif_contr_no_rnc, c.razon_social_no_rnc, c.exit_rnc, c.id_objeto_c, c.num_contrato,
		c.monto_contrato, c.paridad, c.total_contrato, c.fecha_paridad, c.rif_organoente,
		 nto.desc_status_snc, c.snc, acc.desc_acciones, art.desc_articulo113, adj.desc_adjudicado,
		 obj.desc_objeto_contrata, org.descripcion as desc_org, org.filiar, org.id_organoenteads ,p.descripcion ,p.rif as filiares
        from  publicaciones.acciones_llamados c
        JOIN comisiones.notificacion_comision nto ON nto.id_status_snc = c.snc
        JOIN publicaciones.acciones acc ON acc.id_acciones = c.id_accion_cargar
        JOIN publicaciones.adjudicado  adj ON adj.id_adjudicado = c.adjudicado
        JOIN publicaciones.articulo113 art ON art.id_articulo113 = c.id_articulo_113
        JOIN programacion.objeto_contrata  obj ON obj.id_objeto_contrata = c.id_objeto_c
        JOIN public.organoente org  ON c.rif_organoente = org.rif
        JOIN public.organoente p ON p.id_organoente = org.id_organoenteads



        where c.numero_proceso = '$cedula' 
    ");
	...
	public function eliminarPorNumeroProceso($numero_proceso)
	{
		$id_usuario = $this->session->userdata('id_user');

		if (empty($id_usuario)) {
			log_message('error', 'Intento de eliminación sin usuario autenticado: ' . $numero_proceso);
			return false;
		}
		...
		try {
            ...
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'auditoria' AND sequencename = 'llcaccionesdelet_id_llc_seq') THEN
                    CREATE SEQUENCE auditoria.llcaccionesdelet_id_llc_seq;
                    PERFORM setval('auditoria.llcaccionesdelet_id_llc_seq', COALESCE((SELECT MAX(id_llc) FROM auditoria.llcaccionesdelet), 1));
                END IF;
            END $$;
        ");

			// Insertar en auditoría
			$sql_auditoria = "INSERT INTO auditoria.llcaccionesdelet 
                         (id_llc, numero_proceso, id_usuario, date_create) 
                         VALUES (nextval('auditoria.llcaccionesdelet_id_llc_seq'), ?, ?, NOW())";

			$this->db->query($sql_auditoria, array($numero_proceso, $id_usuario));

			// Eliminar registro
			$this->db->query(
				"DELETE FROM publicaciones.acciones_llamados WHERE numero_proceso = ?",
				array($numero_proceso)
			);
			...
			log_message('error', 'Error en eliminarPorNumeroProceso: ' . $e->getMessage());
			return false;
		}
	}
}
