...
class Programacion extends CI_Controller
{
    ...
    public function validadanio()
    { //validar existencia js
    ...
    public function rendir_cero()
    {
        ...
        $data = $this->Programacion_model->rendir_cero($data, $des_unidad, $codigo_onapre, $rif, $data2, $data3, $data4, $data5, $observacion);
        print_r($data);
        die;
        //echo json_encode($data);
    }
    ...
    public function ver_rendicion_realizadas()
    { //////////visualiza las rendiciones realizadas
        if (!$this->session->userdata('session')) redirect('login');
    ...
    public function ver_rendicion_realizadas1()
    { //////////visualiza las rendiciones realizadas
        if (!$this->session->userdata('session')) redirect('login');
    ...
    public function ver_rendicion_realizadas2()
    { //////////visualiza las rendiciones realizadas
        if (!$this->session->userdata('session')) redirect('login');
    ...
    public function ver_rendicion_realizadas3()
    { //////////visualiza las rendiciones realizadas
        if (!$this->session->userdata('session')) redirect('login');
    ...
    public function ver_rendicion_realizadas4()
    { //////////visualiza las rendiciones realizadas
        if (!$this->session->userdata('session')) redirect('login');
    ...
    public function ver_programacion_final()
   ...
   '), 0, 'L');
    ...
    public function read_send_organos_entes() //hacer un pdf de comprobante programacion final para los organos y entes
    { //programacion
        //  $data['ver_programaciones'] = $this->Programacion_model->consultar_reprogramacion($unidad);
        //Se agrega la clase desde thirdparty para usar FPDF
        require_once APPPATH . 'third_party/fpdf/fpdf.php';
   ...
   '), 0, 'L');
    ...
    public function read_send_snc12() //esto es nuevo ya que se coloco para ver los comprobante en snc ver programaciones enviadas ultimo 2025
    {
        ...
        // $id_programacion = $this->input->get('id');
        $date6e = $this->Programacion_model->read_sending_p2_snc($id_ainf_enviada);
        if ($date6e != '') {
            foreach ($date6e as $dati8) {
        ...
        $pdf->Cell(60, 5, utf8_decode('Código ONAPRE:'), 0, 'L');
        //$id_programacion = $this->input->get('id');
        $dat34 = $this->Programacion_model->read_sending_p2_snc($id_programacion);
        if ($dat34 != '') {
            foreach ($dat34 as $dt4r) {
   ...
   '), 0, 'L');
        ...
        $id_programacion = $separar[1]; // Accede al segundo elemento del array
        //    $dat5e = $this->Programacion_model->read_sending_p2_snc($id_ainf_enviada); 
        $dat = $this->Programacion_model->read_sending_rendi_obs($id_ainf_enviada);
        if ($dat != '') {
            foreach ($dat as $dats) {
                $pdf->Cell(30, 5, 'Observaciones:', 0, 'C');

                $pdf->MultiCell(170, 5,  utf8_decode($dats->observacion), 0, 'L');
            }
        }
        $pdf->Ln(1);
    ...
    public function surrender()
    {
        ...
        $data['totalespartida'] = $this->Programacion_model->total_por_partidas($data['id_programacion']);
        // $data['mat'] = $this->Programacion_model->consulta_items($data['id_programacion']);
        // Obtener trimestres ya declarados en cero para este año y programación
        $data['declared_zero'] = $this->Programacion_model->get_declared_zero_trimesters(
    ...
    public function consulta_item_edit()
    { ///consulta para editar factura de la rendicion
        if (!$this->session->userdata('session')) redirect('login');
        $data = $this->input->post();
    ...
    public function read_sendV1() //hacer un pdf de comprobante programacion final para ver por el snc
    { //programacion
        //  $data['ver_programaciones'] = $this->Programacion_model->consultar_reprogramacion($unidad);
        //Se agrega la clase desde thirdparty para usar FPDF
        require_once APPPATH . 'third_party/fpdf/fpdf.php';
   ...
   '), 0, 'L');
        ...
        $data = $this->Programacion_model->consulta_total_objeto_accv1($id_programacion);
        if ($data != '') {
            foreach ($data as $d) {
                $pdf->SetFont('Arial', '', 10);
